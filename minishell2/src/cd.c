...
static void update_cd_env(info_t *info, char *old_dir)
{
    int old_index = my_getenv_index("OLDPWD", info->env);
    int new_index = my_getenv_index("PWD", info->env);

    replace_env(old_index, "OLDPWD", old_dir, info);
    replace_env(new_index, "PWD", info->cur_dir, info);
}
...
int run_cd(info_t *info)
{
    char *val = NULL;
    int update = 0;
    char *old_dir = my_strdup(info->cur_dir);

    get_val(&val, info->input);
    what_cd(info, &update, val);
    if (update) {
        chdir(info->cur_dir);
        update_cd_env(info, old_dir);
    }
    free(old_dir);
    return 0;
}
